...
//       {/* SUCCESS MESSAGE */}
//       {success && <div className="text-green-600 text-sm font-medium">Gửi liên hệ thành công! (37)</div>}
...
export default function ContactForm() {
  ...
  // VALIDATE
  const validate = () => {
    if (!form.full_name) return t("validation.fullNameRequired");
    if (!form.phone_number) return t("validation.phoneNumberRequired");
    if (!form.email) return t("validation.emailRequired");
    if (!form.message) return t("validation.messageRequired");

    const emailRegex = /^\S+@\S+$/i;
    if (!emailRegex.test(form.email)) return t("validation.invalidEmail");
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      {/* SUCCESS */}
      {success && <div className="text-green-600 text-sm font-medium">{t("success")}</div>}
