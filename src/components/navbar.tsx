      ...
      {header.cluster1.map((item, index) => {
        return (
          <span key={index} className="first:pl-0 group px-20 pb-6  ">
            ...
            <div
              className={`opacity-0 invisible group-hover:visible  w-full py-10 left-0 -z-10  top-[80%] group-hover:top-[100%]   shadow-[0_0px_4px_3px_rgba(0,0,0,0.1)]  group-hover:z-50 bg-white   duration-300 group-hover:opacity-100   text-[12px] max-w-[100vw] absolute    `}
            >
                ...
                {item.value.map((value, index1) => {
                  ...
                  return (
                    <div key={index1} className="px-4 first:-ml-4 ">
                      <span className="font-semibold">{value.key1}</span>
                      <div className="mt-2 ">
                        {value.value1.map((value2, index2) => {
                          return (
                            <Link key={index2} href={`/category/${value2._id}?group=${index1 == 0 && "group"}`}>
